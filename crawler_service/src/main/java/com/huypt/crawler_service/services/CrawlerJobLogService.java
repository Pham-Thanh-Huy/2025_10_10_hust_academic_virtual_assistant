...
public class CrawlerJobLogService {
    ...
    public CrawlerJobLog create(
        ...
        jobLog.getLogs().add("Job đã được đưa vào hàng đợi");

        return repository.save(jobLog);
    }

    public void attachJobRunrId(String id, String jobRunrId) {
    ...
    public void markSuccess(String id, int total, int saved) {
        CrawlerJobLog jobLog = getById(id);
        ...
        jobLog.setDurationMs(calculateDuration(
                jobLog.getStartedAt(),
                finishedAt
        ));
        jobLog.getLogs().add(
                "Crawler hoàn thành. Thu thập: "
                        + total + ", cập nhật: " + saved
        );

        repository.save(jobLog);
    }

    public void markFailed(String id, Throwable throwable) {
        CrawlerJobLog jobLog = getById(id);
        Instant finishedAt = Instant.now();
        ...
        jobLog.setErrorMessage(throwable.getMessage());
        jobLog.getLogs().add(
                "Job thất bại: " + throwable.getMessage()
        );

        repository.save(jobLog);
    }

    private long calculateDuration(Instant start, Instant finish) {
        if (start == null) {
            return 0;
        }
