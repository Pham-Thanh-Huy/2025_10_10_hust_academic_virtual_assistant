            ...
            <div className="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-sm">
                <div className="hidden overflow-x-auto lg:block">
                    <table className="w-full">
                        ...
                        <tr className="bg-gray-50/70">
                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500">Câu hỏi</th>
                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500">Người dùng</th>
                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500">Mô hình</th>
                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500">Trạng thái</th>
                            <th className="px-6 py-4 text-left text-sm font-semibold text-gray-500">Thời gian</th>
                            ...
                            chats.map((chat) => (
                                <tr key={chat.id} className="border-t border-gray-100 transition hover:bg-red-50/40">
                                    <td className="max-w-sm px-6 py-5">
                                        <p title={chat.message} className="truncate font-semibold text-gray-900">
                                            {chat.message || '—'}
                                        </p>
                                        <p className="mt-1 truncate text-xs text-gray-400">Session: {chat.sessionId}</p>
                                    </td>

                                    <td className="max-w-56 px-6 py-5">
                                        <p title={chat.username} className="truncate text-sm text-gray-700">
                                            {chat.username || '—'}
                                        </p>
                                    </td>

                                    <td className="px-6 py-5">
                                        <span className="whitespace-nowrap rounded-lg bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700">{chat.model || '—'}</span>
                                    </td>
                                    ...
                                    <td className="whitespace-nowrap px-6 py-5 text-sm text-gray-600">{chat.chatAt || '—'}</td>
              ...
              <span>
                Trang <strong className="font-semibold text-gray-700">{page + 1}/{totalPages}</strong> · Hiển thị{' '}
                  <strong className="font-semibold text-gray-700">{numberOfElements}/{totalElements.toLocaleString('vi-VN')}</strong> lịch sử
              </span>
...
function StatusFilter({
    ...
    return (
        <label className="block">
            <span className="mb-2 block text-sm font-semibold text-gray-700">Trạng thái</span>

            <div className="relative">
                <select value={value} onChange={(event) => onChange(event.target.value as '' | ChatStatus)} className="h-12 w-full appearance-none rounded-xl border border-gray-200 bg-white px-4 pr-10 text-sm text-gray-700 outline-none transition focus:border-red-300 focus:ring-2 focus:ring-red-100">
    ...
    return (
        <label className="block">
            ...
            <div className="flex h-12 items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 transition focus-within:border-red-300 focus-within:ring-2 focus-within:ring-red-100">
                ...
                <input type="datetime-local" step="1" value={value} onChange={(event) => onChange(event.target.value)} className="w-full min-w-0 bg-transparent text-sm text-gray-700 outline-none" />
            </div>
        </label>
    );
}
...
function StatusBadge({ status }: { status: ChatStatus }) {
    return (
        <span className={`inline-flex whitespace-nowrap rounded-full px-3 py-1 text-xs font-semibold ${status === 'ACTIVE' ? 'bg-green-50 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
