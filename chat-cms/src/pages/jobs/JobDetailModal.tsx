    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
      <div className="flex max-h-[90vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b px-6 py-5">
          <div>
            <h2 className="text-xl font-bold text-gray-900">Chi tiết Job</h2>
        ...
        <div className="overflow-y-auto p-6 scrollbar-thin scrollbar-thumb-red-300">
          ...
          <div className="mt-6 rounded-2xl bg-gray-50 p-5">
            <div className="mb-3 flex items-center gap-2">
              {job.status === 'SUCCESS' ? <CheckCircle size={18} className="text-green-600" /> : <Clock size={18} className="text-blue-600" />}

              <span className="font-semibold">Trạng thái hiện tại</span>
            </div>

            <span
              className={`inline-flex rounded-full px-4 py-2 text-sm font-semibold ${job.status === 'SUCCESS' && 'bg-green-100 text-green-700'} ${job.status === 'FAILED' && 'bg-red-100 text-red-700'} ${job.status === 'RUNNING' && 'bg-blue-100 text-blue-700'} ${job.status === 'WAITING' && 'bg-orange-100 text-orange-700'}`}
            >
              {job.status}
            </span>
          </div>
