      ...
      <div className="min-h-screen space-y-6 bg-slate-50 p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <button type="button" onClick={() => setShowTriggerConfirm(true)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[rgb(154,0,31)] px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-red-950/15 transition hover:bg-[rgb(120,0,25)] active:scale-[0.98]">
              <Play size={17} />
              Chạy Job
            </button>

            <button type="button" disabled={loading} onClick={handleRefresh} className="inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-red-200 hover:bg-red-50 hover:text-[rgb(154,0,31)] disabled:cursor-not-allowed disabled:opacity-60">
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
            <FilterField label="Tên Job">
              <div className="relative">
                ...
                <input type="text" value={jobName} placeholder="Tìm kiếm học phần" className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-11 pr-4 text-sm text-slate-700 outline-none transition focus:border-[rgb(154,0,31)] focus:ring-4 focus:ring-red-50" onChange={(event) => setJobName(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && handleSearch()} />
              </div>
            </FilterField>

            <FilterField label="Trạng thái">
              <select value={status} className="input-job" onChange={(event) => setStatus(event.target.value as JobStatus | '')}>
            ...
            <FilterField label="Loại trigger">
              <select value={trigger} className="input-job" onChange={(event) => setTrigger(event.target.value as JobTrigger | '')}>
            ...
            <FilterField label="Từ ngày">
              <input type="date" value={startDate} className="input-job" onChange={(event) => setStartDate(event.target.value)} />
            </FilterField>

            <FilterField label="Đến ngày">
              <input type="date" value={endDate} className="input-job" onChange={(event) => setEndDate(event.target.value)} />
            </FilterField>
          </div>

          <div className="mt-5 flex justify-end border-t border-slate-100 pt-5">
            <button type="button" disabled={loading} onClick={handleSearch} className="inline-flex min-w-[180px] items-center justify-center gap-2 rounded-xl bg-[rgb(154,0,31)] px-7 py-3 text-sm font-semibold text-white transition hover:bg-[rgb(120,0,25)] disabled:opacity-60">
        ...
        <div className="overflow-hidden rounded-3xl border border-slate-100 bg-white shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[1150px] table-fixed">
              ...
              <tr>
                <th className="w-[25%] px-6 py-4 text-left table-heading">Job</th>
                <th className="w-[13%] px-6 py-4 text-left table-heading">Trigger</th>
                <th className="w-[15%] px-6 py-4 text-left table-heading">Trạng thái</th>
                <th className="w-[20%] px-6 py-4 text-left table-heading">Thời gian</th>
                <th className="w-[10%] px-6 py-4 text-left table-heading">Bản ghi</th>
                <th className="w-[17%] px-6 py-4 text-center table-heading">Thao tác</th>
              </tr>
              </thead>
                ...
                return (
                    <tr key={job.id} className="border-t border-slate-100 transition hover:bg-red-50/30">
                      <td className="px-6 py-5">
                        <p className="truncate font-semibold text-slate-900">{job.jobName}</p>
                      ...
                      <td className="px-6 py-5">
                        <div className="flex items-center justify-center gap-2">
                          <ActionButton title="Xem chi tiết" disabled={detailLoadingId === job.id} onClick={() => void handleViewDetail(job.id)} className="bg-red-50 text-[rgb(154,0,31)] hover:bg-red-100">
                            {detailLoadingId === job.id ? <LoaderCircle size={17} className="animate-spin" /> : <Eye size={17} />}
                          </ActionButton>
...
function ConfirmModal({ type, loading, job, close, confirm }: {
  ...
  return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm" onClick={loading ? undefined : close}>
        <div role="dialog" aria-modal="true" className="w-full max-w-md rounded-3xl bg-white p-6 shadow-[0_24px_80px_rgba(15,23,42,0.3)]" onClick={(event) => event.stopPropagation()}>
          <div className="flex items-start justify-between">
            <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${content.iconClass}`}>
              <Icon size={23} />
            </div>

            <button type="button" disabled={loading} onClick={close} className="flex h-9 w-9 items-center justify-center rounded-xl text-slate-400 hover:bg-slate-100 disabled:opacity-50">
              <X size={18} />
            </button>
          </div>

          <h2 className="mt-5 text-xl font-bold text-slate-900">{content.title}</h2>
          <p className="mt-2 text-sm leading-6 text-slate-500">{content.message}</p>

          <div className="mt-7 flex justify-end gap-3">
            <button type="button" disabled={loading} onClick={close} className="rounded-xl border border-slate-200 px-5 py-2.5 text-sm font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-50">
              Không
            </button>

            <button type="button" disabled={loading} onClick={confirm} className={`inline-flex min-w-[120px] items-center justify-center gap-2 rounded-xl px-5 py-2.5 text-sm font-semibold text-white disabled:opacity-60 ${content.buttonClass}`}>
              {loading ? <LoaderCircle size={17} className="animate-spin" /> : <Icon size={16} />}
...
function Pagination({ page, pageSize, totalPages, totalElements, currentElements, loading, changePage, changePageSize }: {
  page: number;
  pageSize: number;
  totalPages: number;
      ...
      <div className="flex flex-col gap-4 border-t border-slate-100 px-6 py-4 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex flex-wrap items-center gap-4">
          <p className="text-sm text-slate-500">
            Trang {totalPages === 0 ? 0 : page + 1}/{totalPages} · Hiển thị {currentElements}/{totalElements.toLocaleString('vi-VN')} Job
          </p>

          <div className="flex items-center gap-2">
            <label htmlFor="job-page-size" className="text-sm text-slate-500">Số dòng:</label>

            <select id="job-page-size" value={pageSize} disabled={loading} onChange={(event) => changePageSize(Number(event.target.value))} className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600 outline-none focus:border-[rgb(154,0,31)] disabled:opacity-50">
...
function PageButton({ disabled, onClick, children }: { disabled: boolean; onClick: () => void; children: ReactNode }) {
  return (
      <button type="button" disabled={disabled} onClick={onClick} className="inline-flex h-10 items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-600 hover:bg-red-50 disabled:cursor-not-allowed disabled:opacity-40">
        {children}
      </button>
  );
}
...
function ActionButton({ title, disabled, onClick, className, children }: {
  ...
  return (
      <button type="button" title={title} disabled={disabled} onClick={onClick} className={`inline-flex h-10 w-10 items-center justify-center rounded-xl transition disabled:opacity-50 ${className}`}>
        {children}
      </button>
  );
}
