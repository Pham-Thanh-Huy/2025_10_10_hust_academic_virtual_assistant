    ...
    <div className="space-y-6 lg:space-y-8">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3 lg:gap-6">
        {overviewCards.map((card) => {
          ...
          return (
            <div key={card.title} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm transition hover:shadow-lg md:p-6">
              <div className="flex justify-between">
                <div>
                  <p className="text-sm text-gray-500">{card.title}</p>

                  <h2 className="mt-3 text-2xl font-bold text-gray-900 md:text-3xl">{card.value}</h2>

                  <p className="mt-2 text-xs font-medium text-green-600">{card.change} so với tháng trước</p>
                </div>
          ...
          return (
            <div key={item.title} className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">Câu hỏi {item.title.toLowerCase()}</p>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">Thống kê câu hỏi 7 ngày gần nhất</p>
          </div>
        ...
        <div className="h-64 md:h-80">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <XAxis dataKey="day" />
              <YAxis />
              <Tooltip formatter={(value) => [`${value} lượt`, 'Tổng số lượt hỏi đáp']} />

              <Line type="monotone" dataKey="total" name="Tổng số lượt hỏi đáp" stroke="#9A001F" strokeWidth={3} dot={{ fill: '#9A001F' }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm md:p-6">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-400">Các câu hỏi mới nhất từ sinh viên</p>
          </div>
        </div>
          ...
          <table className="min-w-[650px] w-full">
            ...
            <tbody>
              {recentQuestions.map((item) => (
                <tr key={item.id} className="group border-b border-gray-100 transition-all duration-200 hover:bg-gray-50 hover:shadow-sm">
                  {/* User */}
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-3">
                      <div className="flex h-10 w-10 items-center justify-center rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 text-sm font-bold text-white">{item.user?.charAt(0).toUpperCase()}</div>
                  ...
                  {/* Question */}
                  <td className="px-6 py-5">
                    <div className="max-w-md rounded-xl bg-red-50 px-4 py-2 text-sm text-red-700 shadow-sm transition group-hover:bg-red-100">
                  ...
                  {/* Time */}
                  <td className="px-6 py-5">
                    <span className="rounded-lg bg-gray-100 px-3 py-1.5 text-xs font-medium text-gray-500">{item.time}</span>
                  </td>
                </tr>
              ))}
            </tbody>
